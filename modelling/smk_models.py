"""
Module with GP model definitions using various kernels. Taken from:
https://github.com/SebastianCallh/blog-spectral-mixture-kernel/blob/master/src/smk/models.py
"""
...
class GP(gp.models.ExactGP):
    def __init__(self, cov, train_x, train_y, noise):
        super(GP, self).__init__(train_x, train_y, FixedNoiseGaussianLikelihood(noise))
        self.mean = gp.means.ConstantMean()
        self.cov = cov
...
class SMKernelGP(GP):
    def __init__(self, train_x, train_y, noise, num_mixtures=10):
        kernel = SpectralMixtureKernel(num_mixtures)
        kernel.initialize_from_data(train_x, train_y)

        super(SMKernelGP, self).__init__(kernel, train_x, train_y, noise)
        self.mean = gp.means.ConstantMean()
        self.cov = kernel
...
class CompositeKernelGP(GP):
    def __init__(self, train_x, train_y, noise, num_mixtures=10):
        ...
        super(CompositeKernelGP, self).__init__(kernel, train_x, train_y, noise)
        self.mean = gp.means.ConstantMean()
        self.smk = smk
